---
name: create-mode
description: Create a new mode (atomic cognitive action) for the skills framework
argument-hint: "[mode name and purpose]"
---

# Create Mode

Create a well-scoped, composable mode that does one thing well.

## When to Use

Use this skill when:
- A recurring cognitive action appears across workflows
- The action has clear preconditions/postconditions
- The action is atomic (can't be meaningfully decomposed)
- The action is distinct from existing modes

Do NOT use when:
- It's a variation of an existing mode (extend instead)
- It requires multiple cognitive actions (that's a workflow)
- It's tool-specific (belongs in a skill)

## Process

Follow the workflow defined in `workflows/create_mode_workflow.md`.

**Summary:**

1. **Understand existing modes** — Check modes/ for overlap
2. **Define the mode** — Name, goal, intent, scope, category
3. **Write the mode** — Create file in modes/ with required sections
4. **Validate** — Ensure distinct purpose, atomic, self-contained scope

## Key Principles

- **Atomic**: One cognitive action, not decomposable further
- **Composable**: Can be sequenced into workflows
- **Self-contained**: Scope doesn't reference other modes
- **Goal-oriented**: Defined by outcomes, not steps
- **Declarative**: Define goals and key results, not required steps

## Existing Modes

Before creating, check for overlap:

| Category | Modes |
|----------|------------|
| Understanding | orient, define, investigate |
| Planning | plan, design, brainstorm |
| Execution | implement |
| Verification | validate, critique |
| Maintenance | bookkeep, upkeep |

## Output

New mode document in `modes/[name].md`
