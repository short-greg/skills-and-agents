...
1. **Check recovery state** per `protocols/tracking_and_recovery.md` — if progress file exists, read state and resume from last completed task
...
5. **Output task plan** — using `planning` primitive, sequence selected tasks with dependencies, identify parallel opportunities
...
3. If no fallback: diagnose failure using the `investigating` primitive
