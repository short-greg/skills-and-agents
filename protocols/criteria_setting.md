...
- [Artifacts and Outputs](#artifacts-and-outputs) — Criteria Specification, Rubric, Threshold Definition, Examples
- [Core Approaches](#core-approaches) — Specification Techniques, Categorization Techniques, Refinement Techniques
- [Example Patterns](#example-patterns) — Establishing Acceptance Criteria, Making Vague Criteria Concrete
...
|-----------------|---------|-------------|-----------|
| **Criteria Specification** | Formal statement of verifiable criteria | When documenting acceptance conditions | Either |
| **Rubric** | Multi-dimensional assessment template with scoring | When evaluating across multiple dimensions | Yes |
| **Threshold Definition** | Explicit cutoff values with units and rationale | When defining pass/fail boundaries | Output |
| **Examples** | Positive/negative examples illustrating boundaries | When grounding abstract criteria | Output |
...
| Technique | When | Why | How | Output/Artifact | Positive Validation | Negative Validation |
|-----------|------|-----|-----|-----------------|---------------------|---------------------|
| **Binary Criteria** | Classification is simple yes/no decision | To create unambiguous pass/fail criteria | Define condition that must be met, specify exactly what constitutes pass vs fail, provide examples of each, test with edge cases | Criteria Specification | Is pass/fail boundary clear? Are edge cases handled? | Are there gray areas where pass/fail is ambiguous? |
| **Scale Criteria** | Quality exists on a continuum | To enable gradations of quality assessment | Define scale points (e.g., 1-5, poor-excellent), specify indicators for each level, provide examples at each level, establish how to assign ratings | Rubric | Are scale points defined? Is each level distinguishable? | Are adjacent levels indistinguishable from each other? |
| **Categorical Criteria** | Multiple distinct categories exist | To classify into discrete types or levels | List all possible categories, define membership criteria for each, ensure categories are mutually exclusive, specify how to handle edge cases | Criteria Specification | Are categories mutually exclusive? Is classification unambiguous? | Do items fit multiple categories or none? |
| **Rubric Criteria** | Multiple dimensions need assessment | To evaluate across multiple independent criteria | Identify evaluation dimensions, define criteria for each dimension, assign weights if needed, specify scoring method, create assessment template | Rubric | Are all dimensions covered? Are criteria independent? | Are dimensions overlapping or redundant? |
| **Aggregation Logic** | Multiple criteria must combine into overall verdict | To specify how individual criteria combine | Choose logic: AND (all must pass), OR (any can pass), weighted (some matter more), threshold (X of Y must pass), document aggregation rule explicitly | Criteria Specification | Is aggregation logic explicit? Is overall verdict determinable? | Is it unclear how criteria combine? Can different people reach different overall verdicts? |
